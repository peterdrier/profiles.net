...
- [`service-read-no-ef`](architecture/service-read-no-ef.md) — `I*Read` interfaces must use DTO/Info projections only — no `Humans.Domain.Entities.*`, no `Microsoft.EntityFrameworkCore.*`, no `IQueryable` in any method signature. Enforced by HUM0029.
