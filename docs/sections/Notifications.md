...
- **Camps:** `ICampServiceRead.GetSettingsAsync` + `GetCampsForYearAsync(year)` — per-camp-lead pending-requests meter (count derived in-memory from the returned camp snapshots).
