...
**Status:** (A) Fully migrated. All four services live in `Humans.Application.Services.Shifts` and route through `IShiftManagementRepository` / `IShiftSignupRepository` / `IVolunteerTrackingRepository`. Cross-domain navs on Shifts-owned entities deleted 2026-04-25 in nobodies-collective/Humans#541 final pass; FKs stay wired in EF via the typed-FK form.

- Services live in `Humans.Application.Services.Shifts/` and never import `Microsoft.EntityFrameworkCore`.
- `IShiftManagementRepository`, `IShiftSignupRepository`, `IVolunteerTrackingRepository` (impls in `Humans.Infrastructure/Repositories/Shifts/`) are the only code paths touching this section's tables via `DbContext`.
- **Caching:**
...
- **`IShiftManagementRepository`** (impl: `ShiftRepository`) — owns `rotas`, `shifts`, `event_settings`, `shift_tags`, `volunteer_tag_preferences`, `rota_shift_tags`, `volunteer_event_profiles`.
...
- **`IShiftSignupRepository`** (impl: `ShiftRepository`) — owns `shift_signups`. Also surfaces a read for `volunteer_event_profiles` used by the GDPR contributor (`GetVolunteerEventProfilesForUserAsync`).
