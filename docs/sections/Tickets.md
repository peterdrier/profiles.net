...
- **Budget:** `IBudgetService` — `GetActiveYearAsync` + `ComputeBudgetSummary` feed the dashboard's break-even calculation; `TicketingBudgetService` (a Tickets-section bridge) reads paid-order data via `ITicketServiceRead.GetTicketOrdersAsync` and pushes the weekly actuals/projection writes into Budget through `IBudgetService` (`SyncTicketingActualsAsync` etc.) without ever touching `budget_*` tables directly. (Pre-#815 the bridge had its own `ITicketingBudgetRepository`; that was removed and the read now flows through `ITicketServiceRead`.)
...
**Status:** (A) Fully §15-compliant. All section services live in `Humans.Application.Services.Tickets`. `TicketQueryService` and `TicketSyncService` route every database read/write through `ITicketRepository`; `TicketingBudgetService` is a repository-free bridge that reads paid orders via `ITicketServiceRead.GetTicketOrdersAsync` and delegates all Budget-owned writes to `IBudgetService`. Neither `TicketQueryService.cs`, `TicketSyncService.cs`, nor `TicketingBudgetService.cs` imports `Microsoft.EntityFrameworkCore` or references `HumansDbContext`. Umbrella issue nobodies-collective/Humans#545 closed by sub-tasks #545a (TicketQueryService → Application), #545b (TicketingBudgetService → Application, originally with its own `ITicketingBudgetRepository` — that repository was later removed in #815, leaving the bridge reading via `ITicketServiceRead`), #545c (TicketSyncService → Application + `IShiftManagementService` / `IUserService` routing). `ITicketVendorService` connector split landed in peterdrier/Humans PR #277.

**Caching:** the §15 caching decorator pattern is applied (T-07, 2026-05-16). `CachingTicketQueryService` (Infrastructure, Singleton) wraps a keyed-inner `TicketQueryService` (Application, Scoped) and composes two tracked slices: `OrdersCache : TrackedCache<Guid, TicketOrderInfo>` (with `warmOnStartup: true`) keyed by ticket order id, and `UserHoldingsCache : TrackedCache<Guid, CachedUserTicketHoldings>` keyed by user id. The order slice owns the per-order `TicketOrderInfo` projection with attendees embedded; the user slice owns `UserTicketHoldings` and stores the 5-minute freshness deadline in the tracked value. The warm path calls the normal `ITicketService.GetTicketOrdersAsync` read method on the keyed inner, not a cache-named helper. Composition (multiple inner caches with different shapes) precludes inheriting `TrackedCache` directly, so the decorator implements `IHostedService` itself and forwards `StartAsync` to both tracked caches; the user slice has `warmOnStartup: false`. The only remaining `IMemoryCache` ticket entry owned by the decorator is per-event `TicketEventSummary`; the inner is cache-free and an architecture test pins no `IMemoryCache` in the inner's constructor.

**Architecture tests:**
- `tests/Humans.Application.Tests/Architecture/TicketQueryArchitectureTests.cs` — pins namespace, no DbContext, `ITicketRepository` required, cross-section reads via service interfaces only, **no `IMemoryCache` on the inner** (T-07), and the decorator implements both `ITicketService` / `ITicketServiceRead` and `ITicketCacheInvalidator`.
...
- `tests/Humans.Application.Tests/Architecture/TicketingBudgetArchitectureTests.cs` — documents the bridge as reading via `ITicketServiceRead` and delegating to `IBudgetService`; asserts the `TicketingBudgetService` constructor takes no Store type.
...
The Tickets→Budget bridge owns no repository: `TicketingBudgetService` reads paid-order data through `ITicketServiceRead` (served by `ITicketRepository` on the read side) and delegates all writes to `IBudgetService`. (The dedicated `ITicketingBudgetRepository` it once used was removed in #815.)
...
- When extending the Tickets→Budget bridge, source new read data from `ITicketServiceRead` (adding methods there only if the existing `GetTicketOrdersAsync` read model is insufficient). Projection/line-item writes remain Budget-owned and must route through `IBudgetService`.
