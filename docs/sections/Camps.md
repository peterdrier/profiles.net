...
- A **Camp Lead** is a human responsible for managing a camp. Lead authorization flows **solely** through a `CampRoleAssignment` against the `CampRoleDefinition` whose `SpecialRole = CampSpecialRole.Lead` (exposed on the read model as `CampSeasonInfo.LeadUserIds` and checked via `CampInfo.IsLead`). The legacy `CampLead` entity / `camp_leads` table remains in place transitionally but is **no longer consulted for lead authorization** — only the seed-migration snapshot and the GDPR export still read it; the entity and table are dropped in a follow-up PR (issue nobodies-collective/Humans#774).
...
- A **Camp Role Assignment** is a per-season binding of a `CampMember` to a `CampRoleDefinition`. "Camp Lead" and "Workshop Lead" **are** `CampRoleDefinition` rows (special, `SpecialRole != None`); the legacy `CampLead` entity is retained transitionally but is no longer read by the auth handler — lead authority is resolved entirely from `CampRoleAssignment` until the follow-up table-drop PR (issue nobodies-collective/Humans#774) lands.
...
- When an account merge accepts, `ICampService.ReassignAsync` moves the source's `CampRoleAssignment` rows to the target by re-FKing `CampRoleAssignment.CampMemberId` from the source's `CampMember` to the target's `CampMember` in the same season (on unique-index collision the target wins and the source assignment is dropped). Because Camp Lead is now a `CampRoleAssignment`, this moves leads too; the legacy `CampLead` table is no longer re-FK'd here. Called only by `IAccountMergeService.AcceptAsync` (Profiles section).
...
- **Profiles:** Called by `IAccountMergeService` (Profiles section) — `ICampService.ReassignAsync` moves the source's `CampRoleAssignment` rows to the target's `CampMember` in the same season during account merge fold (re-FKing `CampRoleAssignment.CampMemberId`, target wins on unique-index collision). Because Camp Lead is a `CampRoleAssignment`, leads move too; the legacy `CampLead` table is no longer re-FK'd.
- **Early Entry contributor:** `CampService` implements `IEarlyEntryProvider` — emits one grant per Active `HasEarlyEntry` member (entry date = global `CampSettings.EeStartDate`, source = "Camp: {name}"). `SetEarlyEntryAsync` and the member-removal cascade evict the per-user EE cache via `IEarlyEntryInvalidator`.
...
- **Caching decorator (T-06).** `CachingCampService` (Singleton, `Humans.Infrastructure.Services.Camps`) wraps `ICampService` per design-rules §15d. It owns a `ConcurrentDictionary<Guid, CampInfo>` keyed by camp id (the canonical per-camp projection) plus a single-slot `CampSettingsInfo`. Year-keyed sub-views (`GetCampsForYearAsync`, the public/placement summaries that `CampApiController` projects from it) are filtered **snapshots** of the canonical cache, not separate entries. Lead checks (`CampInfo.IsLead`, used by the auth handler) answer from the snapshot. The legacy `IMemoryCache`-backed `camps_year_{N}` / `CampSettings` 5-min keys are retired.
